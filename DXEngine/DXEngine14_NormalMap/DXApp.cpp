...
bool DXApp::InitTexture()
{
	...
	if (FAILED(hr))
	{
		MessageBox(NULL, L"노멀 텍스처 로드 실패", L"오류", MB_OK);
		return false;
	}
	...
	for (int ix = 0; ix < childCount; ++ix)
	{
		...
		for (int jx = 0; jx < polyCount; ++jx)
		{
			...
			for (int kx = 0; kx < vertexCount; ++kx)
			{
				...
				// 탄젠트 읽기
				bool hasNoTangent = fbxMesh->GetElementTangentCount() < 1;
				if (hasNoTangent)
				{
					MessageBox(NULL, L"탄젠트 없음", L"오류", MB_OK);
				}
				vertex.tangent = hasNoTangent ? XMFLOAT3(0.0f, 0.0f, 0.0f) : ReadTangent(fbxMesh, vertexIndex, vertexCount);
				
				//바이노멀
				bool hasNoBinormal = fbxMesh->GetElementBinormalCount() < 1;
				if (hasNoBinormal)
				{
					MessageBox(NULL, L"Binormal 없음", L"오류", MB_OK);
				}
				vertex.binormal = hasNoBinormal ? XMFLOAT3(0.0f, 0.0f, 0.0f) : ReadBinormal(fbxMesh, vertexIndex, vertexCount);
