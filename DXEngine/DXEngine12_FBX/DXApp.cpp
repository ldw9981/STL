...
DXApp::DXApp(HINSTANCE hinstance)
{
	...
	clientWidth = 800;
	clientHeight = 600;
	applicationName = L"Engine12 - FBX";
...
int DXApp::Run()
{
	...
	while (msg.message != WM_QUIT)
	{
		...
		else
		{
			...
			double time = gameTimer.DeltaTime();
...
bool DXApp::LoadModel(const char * fileName)
{
	...
	// 배열 공간 확보.
	//vertices.reserve(nVertices);// = new Vertex[nVertices];
	//indices.reserve(nIndices); //= new DWORD[nIndices];
...
HRESULT DXApp::LoadFBX(const char * fileName, std::vector<Vertex>* pOutVertices, std::vector<DWORD>* pOutIndices)
{
	...
	FbxImporter* importer = Fbx;


	if (importer->Initialize(fileName, -1, fbxManager, ))
	{

	}

	if (importer->Importer(fbxScene) == false)
	{
		return E_FAIL;
	}
	...
	for (int ix = 0; ix < childCount; ix++)
	{
		...
		// 노드 속성 타입 확인
		FbxNodeAttribute::EType attrType = fbxChildNode->GetNodeAttribute()->GetAttributeType();
		...
		//폴리곤 수 확인
		int polyCount = fbxMesh->GetPolyCount();
		int vertexCounter = 0;
		for (int jx = 0; jx < polyCount; jx++)
		{
			// 폴리곤 구성 점점 재수확인
			int vertexCount = fbxMesh->GetPolyonSize();	//3이어야한다, 

			for (int kx = 0; kx < vertexCount; kx++)
			{
				int vertexIndex = fbxMesh->GetPolysonVertex(jx, kx);
				//정점 정보 읽기.
				Vertex vertex;
