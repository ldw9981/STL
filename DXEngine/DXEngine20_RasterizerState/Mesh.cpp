...
bool Mesh::CreateRasterizerState(ID3D11Device * device, D3D11_FILL_MODE fillMode, D3D11_CULL_MODE cullMode)
{
	if (vertexShader.CreateRasterizerState(device,fillMode,cullMode))
		return true;
...
{
	vertexShader.BindRasterizerState(deviceContext);
}
