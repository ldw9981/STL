...
namespace render
{
	...
	bool IsInPlayerMovableRect(COORD input)
	{
        if(updateScreenSize.Left > input.X )
            return false;

		if (updateScreenSize.Right < input.X)
			return false;

		if (updateScreenSize.Top > input.Y)
			return false;

		if (updateScreenSize.Bottom < input.Y)
			return false;
